 ...
 *
 * Styled with Garden Springs Wellness tokens (forest / gold / sand / muted) so
 * it matches the LeadForm inputs it lives alongside.
 */
    ...
    <div ref={wrapRef} className="relative">
      <div className="relative">
        ...
        <button
          ...
          className="absolute inset-y-0 right-0 flex items-center pr-3 text-muted transition-colors hover:text-forest-700"
        >
          <ChevronDown
      ...
      {open && matches.length > 0 && (
        <ul
          id={listId}
          role="listbox"
          className="absolute z-20 mt-1.5 max-h-64 w-full overflow-auto rounded-xl border border-forest-900/15 bg-white py-1 shadow-card"
        >
          {matches.map((provider, i) => {
            ...
            return (
              <li
                ...
                className={`flex cursor-pointer items-center justify-between px-4 py-2 text-sm text-forest-900 transition-colors ${
                  i === active ? "bg-forest-50" : ""
                }`}
              >
                <span>{provider}</span>
                {selected && <Check className="h-4 w-4 text-forest-600" />}
              </li>
            );
          })}
        </ul>
